    ...
    <header className="sticky top-0 z-50 backdrop-blur-xl bg-ink/80 border-b border-line">
      <div className="max-w-5xl mx-auto px-5 h-14 sm:h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <Link href="/#katalog" className="btn-acid text-[12px] sm:text-sm font-semibold px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg transition whitespace-nowrap">Top Up</Link>
          <button
            type="button"
            onClick={() => setOpen((v) => !v)}
            aria-label={open ? "Tutup menu" : "Buka menu"}
            className="sm:hidden w-9 h-9 rounded-lg border border-line flex items-center justify-center text-grey hover:text-paper transition"
          >
            {open ? (
      ...
      {open && (
        <nav className="sm:hidden border-t border-line bg-panel/95 backdrop-blur-xl px-5 py-3 flex flex-col gap-1">
          <Link href="/" onClick={() => setOpen(false)} className="py-3 text-sm text-grey hover:text-paper transition rounded-lg px-2">Home</Link>
          {NAV.map((item) => (
